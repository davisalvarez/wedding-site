
const lista = {
    "lista_invitados":[
        {
            "id": 1,
            "url": "mare-y-davis",
            "invitados":[
                {"nombre": "Davis Alvarez"},
                {"nombre": "Marely Ramirez"},
                {"nombre": "Fermin Aldeguer"}
            ]
        },
        {
            "id": 2,
            "url": "mami-y-papi",
            "invitados":[
                {"nombre": "David Alvarez"},
                {"nombre": "Ruth de Alvarez"}
            ]
        },
        {
            "id": 3,
            "url": "keila-y-brandon",
            "invitados":[
                {"nombre": "Keila Alvarez"},
                {"nombre": "Brandon Barrios"}
            ]
        },
        {
            "id": 4,
            "url": "",
            "invitados":[
                {"nombre": ""},
                {"nombre": ""},
                {"nombre": ""},
            ]
        },
        {
            "id": 5,
            "url": "tio-joel-y-tia-carmen",
            "invitados":[
                {"nombre": "Joel Alvarez"},
                {"nombre": "Carmen de Alvarez"}
            ]
        },
        {
            "id": 6,
            "url": "primo-y-alessandra",
            "invitados":[
                {"nombre": "Fernando Alvarez"},
                {"nombre": "Alessandra Sian"}
            ]
        },
        {
            "id": 7,
            "url": "yosi-y-theissen",
            "invitados":[
                {"nombre": "Yoselin Alvarez"},
                {"nombre": "Francisto Theissen"}
            ]
        },
        {
            "id": 8,
            "url": "familia-canle-alvarez",
            "invitados":[
                {"nombre": "Nancy Alvarez"},
                {"nombre": "Jorge Canle"},
                {"nombre": "Ammi Canle"},
                {"nombre": "Briana Canle"}

            ]
        },
        {
            "id": 9,
            "url": "carlos-y-amarilis",
            "invitados":[
                {"nombre": "Carlos Orozco"},
                {"nombre": "Amarilis de Orozco"}
            ]
        },
        {
            "id": 10,
            "url": "familia-zepeda-orozco",
            "invitados":[
                {"nombre": "Maura Orozco"},
                {"nombre": "Diego Zepeda"},
                {"nombre": "Melsy Zepeda"},
            ]
        },
        {
            "id": 11,
            "url": "familia-orozco-solares",
            "invitados":[
                {"nombre": "Ismael Orozco"},
                {"nombre": "Betziada de Orozco"},
                {"nombre": "Jacky Orozco"},
                {"nombre": "Isabela Orozco"}
            ]
        },
        {
            "id": 12,
            "url": "brichch-y-fer",
            "invitados":[
                {"nombre": "Brian Chavez"},
                {"nombre": "Yenifer de Chavez"}
            ]
        },
        {
            "id": 13,
            "url": "edgar-y-susy",
            "invitados":[
                {"nombre": "Edgar Borrayo"},
                {"nombre": "Susy López"}
            ]
        },
        {
            "id": 14,
            "url": "alexi-s-y-monica",
            "invitados":[
                {"nombre": "Alexi Sevilla"},
                {"nombre": "Monica Herrera"}
            ]
        },
        {
            "id": 15,
            "url": "suchi-y-daniela",
            "invitados":[
                {"nombre": "Jorge Suchite"},
                {"nombre": "Daniela Herrarte"}
            ]
        },
        {
            "id": 16,
            "url": "wingz",
            "invitados":[
                {"nombre": "Julio Baraona"}
            ]
        },
        {
            "id": 17,
            "url": "schnawow",
            "invitados":[
                {"nombre": "Daniel Morales"},
                {"nombre": "Y un acompañante"}
            ]
        },
        {
            "id": 18,
            "url": "eric-k",
            "invitados":[
                {"nombre": "Eric Mendoza"},
                {"nombre": "Y un acompañante"},
                {"nombre": ""},
            ]
        },
        {
            "id": 19,
            "url": "robbin-y-andrea",
            "invitados":[
                {"nombre": "Robbin Woods"},
                {"nombre": "Andrea Santos"}
            ]
        },
        {
            "id": 20,
            "url": "isa-y-cobis",
            "invitados":[
                {"nombre": "Luis Cobos"},
                {"nombre": "Isa de Cobos"}
            ]
        },
        {
            "id": 21,
            "url": "kevin",
            "invitados":[
                {"nombre": "Kevin Muñoz"},
                {"nombre": "Y un acompañante"}
            ]
        },
        {
            "id": 22,
            "url": "rul-y-kathleen",
            "invitados":[
                {"nombre": "Raul de Leon"},
                {"nombre": "Kathleen de Leon"},
                {"nombre": ""}
            ]
        },
        {
            "id": 23,
            "url": "chejo",
            "invitados":[
                {"nombre": "Sergio Cevallos"},
                {"nombre": "Y un acompañante"}
            ]
        },
        {
            "id": 24,
            "url": "esteban",
            "invitados":[
                {"nombre": "Esteban Avalos"},
                {"nombre": "Y un acompañante"}
            ]
        },
        {
            "id": 25,
            "url": "roro",
            "invitados":[
                {"nombre": "Rodrigo Villatoro"},
                {"nombre": "Y un acompañante"}
            ]
        },
        {
            "id": 26,
            "url": "gus-y-dulce",
            "invitados":[
                {"nombre": "Gustavo Orellana"},
                {"nombre": "Dulce Gonzalez"}
            ]
        },
        {
            "id": 27,
            "url": "hector",
            "invitados":[
                {"nombre": "Hector Gomez"},
                {"nombre": "Y un acompañante"}
            ]
        },
        {
            "id": 28,
            "url": "kevin-sanchez",
            "invitados":[
                {"nombre": "kevin-sanchez"}
            ]
        },
        {
            "id": 29,
            "url": "cristian-lupe-lucia",
            "invitados":[
                {"nombre": "Cristian Alvarez"},
                {"nombre": "Lupe Alvarez"},
                {"nombre": "Lucia Alvarez"}
            ]
        },
        {
            "id": 30,
            "url": "tia-concha",
            "invitados":[
                {"nombre": "Tía Concha"},
                {"nombre": "Walter Alvarez"},
                {"nombre": "Javier Alvarez"}
            ]
        },
        {
            "id": 31,
            "url": "abel-cony-sury",
            "invitados":[
                {"nombre": "Abel Alvarez"},
                {"nombre": "Cony de Alvarez"},
                {"nombre": "Sury Alvarez"}
            ]
        },
        {
            "id": 32,
            "url": "familia-chiry",
            "invitados":[
                {"nombre": "Nataly Alvarez"},
                {"nombre": "Alejandro Zarceño"},
                {"nombre": "Levy Zarceño"},
                {"nombre": "Martín Zarceño"}
            ]
        },
        {
            "id": 33,
            "url": "raisa-y-erick",
            "invitados":[
                {"nombre": "Raisa de Pérez"},
                {"nombre": "Erick Pérez"}
            ]
        },
        {
            "id": 34,
            "url": "ricardo-y-yoli",
            "invitados":[
                {"nombre": "Ricardo Chavez"},
                {"nombre": "Yoli de Chavez"}
            ]
        },
        {
            "id": 35,
            "url": "familia-garcia",
            "invitados":[
                {"nombre": "Selvyn Garcia"},
                {"nombre": "Thomas Garcia"},
                {"nombre": "Allison Garcia"},
                {"nombre": "Ema Garcia"},
            ]
        },
        {
            "id": 36,
            "url": "familia-diaz",
            "invitados":[
                {"nombre": "Telma Diaz"},
                {"nombre": "Thomas Diaz"},
                {"nombre": "Allison Diaz"},
                {"nombre": "Ema Diaz"}
            ]
        },
        {
            "id": 37,
            "url": "tobal-y-raquel",
            "invitados":[
                {"nombre": "Cristobal Montejo"},
                {"nombre": "Raquel de Montejo"},
                {"nombre": ""}
            ]
        },
        {
            "id": 38,
            "url": "juanito-y-diana",
            "invitados":[
                {"nombre": "Juan Krin"},
                {"nombre": "Diana de Krin"},
                {"nombre": ""}
            ]
        },
        {
            "id": 39,
            "url": "mami",
            "invitados":[
                {"nombre": "Maria Lopez"}
            ]
        },
        {
            "id": 40,
            "url": "familia-ramirez-valdez",
            "invitados":[
                {"nombre": "Cristian Ramirez"},
                {"nombre": "Estefany Valdez"},
                {"nombre": "Thiago Ramirez"},
                {"nombre": "Lucas Ramirez"}
            ]
        },
        {
            "id": 41,
            "url": "familia-fajardo-mendoza",
            "invitados":[
                {"nombre": "Luis Fajardo"},
                {"nombre": "Esther Mendoza"},
                {"nombre": "Nathalie Fajardo"},
                {"nombre": "Isabela Fajardo"}
            ]
        },
        {
            "id": 42,
            "url": "los-carlos",
            "invitados":[
                {"nombre": "Carlos Villatoro"},
                {"nombre": "Carol Ramirez"},
                {"nombre": "Carlos Villatoro"},
                {"nombre": "Carlos Villatoro"}
            ]
        },
        {
            "id": 43,
            "url": "familia-lopez-aquino",
            "invitados":[
                {"nombre": "Manuel Lopez"},
                {"nombre": "Mama Vicky"}
            ]
        },
        {
            "id": 44,
            "url": "javi-marce-july",
            "invitados":[
                {"nombre": "Julisa Alvarez"},
                {"nombre": "Javier Lemus"},
                {"nombre": "Marcela Lemus"}
            ]
        },
        {
            "id": 45,
            "url": "ale-y-dany",
            "invitados":[
                {"nombre": "Daniel Alvarez"},
                {"nombre": "Alejandro Reyes"}
            ]
        },
        {
            "id": 46,
            "url": "michi-y-carlos",
            "invitados":[
                {"nombre": "Mishel Alvarez"},
                {"nombre": "Carlos Vasquez"}
            ]
        },
        {
            "id": 47,
            "url": "Gloria Lopez",
            "invitados":[
                {"nombre": "tia-gloria"}
            ]
        },
        {
            "id": 48,
            "url": "familia-martinez-lopez",
            "invitados":[
                {"nombre": "Rosa Lopez"},
                {"nombre": "Rocael Martinez"},
                {"nombre": "Pablo Martinez"}
            ]
        },
        {
            "id": 49,
            "url": "familia-urbina-lopez",
            "invitados":[
                {"nombre": "Adilis Urbina"},
                {"nombre": "Juan Pablo Urbina"},
                {"nombre": "Pablo Andre Urbina"},
                {"nombre": "Mathias Urbina"}
            ]
        },
        {
            "id": 50,
            "url": "tia-letty",
            "invitados":[
                {"nombre": "Leticia Lopez"}
            ]
        },
        {
            "id": 51,
            "url": "",
            "invitados":[
                {"nombre": ""},
                {"nombre": ""},
                {"nombre": ""}
            ]
        },
        {
            "id": 52,
            "url": "jhon-y-arlinne",
            "invitados":[
                {"nombre": "Jhonatan Alvarez"},
                {"nombre": "Arlynne Garcia"}
            ]
        },
        {
            "id": 53,
            "url": "familia-rosales-citalan",
            "invitados":[
                {"nombre": "Carlos Rosales"},
                {"nombre": "Madelyn de Rosales"},
                {"nombre": "Dulce Rosales"},
                {"nombre": "Allisson Rosales"}
            ]
        },
        {
            "id": 54,
            "url": "milton-david-evelyn",
            "invitados":[
                {"nombre": "Milton Alvarez"},
                {"nombre": "Evelyn Garcia"},
                {"nombre": "David Alvarez"}
            ]
        },
        {
            "id": 55,
            "url": "manuel-ramirez",
            "invitados":[
                {"nombre": "Manuel Ramirez"}
            ]
        },
        {
            "id": 56,
            "url": "ale-oli-dino-aury",
            "invitados":[
                {"nombre": "Dinora Solares"},
                {"nombre": "Alejandro Barillas"},
                {"nombre": "Olvia Barillas"},
                {"nombre": "Aura Herrera"}
            ]
        },
        {
            "id": 57,
            "url": "lic",
            "invitados":[
                {"nombre": "julio-mendoza"}
            ]
        },
        {
            "id": 58,
            "url": "melanie-juarez",
            "invitados":[
                {"nombre": "Melani Juarez"}
            ]
        },
        {
            "id": 59,
            "url": "Sandra Gomez",
            "invitados":[
                {"nombre": "sandra-gomez"}
            ]
        },
        {
            "id": 60,
            "url": "dominic-paty-zoe",
            "invitados":[
                {"nombre": "Patricia Aldana"},
                {"nombre": "Dominic Rodriguez"},
                {"nombre": "Zoe Rodriguez"}
            ]
        },
        {
            "id": 61,
            "url": "ale-cortez",
            "invitados":[
                {"nombre": "Alejandra Cortez"},
                {"nombre": "Y un acompañante"}
            ]
        },
        {
            "id": 62,
            "url": "mariano",
            "invitados":[
                {"nombre": "Mariano Pablo"}
            ]
        },
        {
            "id": 63,
            "url": "familia-solis-roldan",
            "invitados":[
                {"nombre": "Alejandra Roldan"},
                {"nombre": "Jonatan Solis"},
                {"nombre": "Sophia Roldan"}
            ]
        },
        {
            "id": 64,
            "url": "familia-solares-castillo",
            "invitados":[
                {"nombre": "Ps. Luis Pedro Solares"},
                {"nombre": "Ps. Silvia de Solares"},
                {"nombre": "Ana Paula Solares"}
            ]
        },
        {
            "id": 65,
            "url": "familia-herrera-castillo",
            "invitados":[
                {"nombre": "Luis Herrera"},
                {"nombre": "Gilma de Herrera"},
                {"nombre": "Alfredo Herrera"},
                {"nombre": "Javier Herrera"},
                {"nombre": "Doña Gilmita"}
            ]
        },
        {
            "id": 66,
            "url": "familia-de-leon",
            "invitados":[
                {"nombre": "Otto de Leon"},
                {"nombre": "Evelyn de Leon"},
                {"nombre": "Andres de Leon"},
                {"nombre": "Katherine de Leon"}
            ]
        },
        {
            "id": 67,
            "url": "familia-martinez-lemus",
            "invitados":[
                {"nombre": "Hugo Martinez"},
                {"nombre": "Victoria de Martinez"},
                {"nombre": "Lucia Martinez"}
            ]
        },
        {
            "id": 68,
            "url": "richi-y-dany",
            "invitados":[
                {"nombre": "Richie Contreras"},
                {"nombre": "Daniela Martinez"}
            ]
        },
        {
            "id": 69,
            "url": "familia-de-diaz-centeno",
            "invitados":[
                {"nombre": "Samy Diaz"},
                {"nombre": "Aury de Diaz"},
                {"nombre": "Jimena Diaz"}
            ]
        },
        {
            "id": 70,
            "url": "ps-hil",
            "invitados":[
                {"nombre": "Hilberto Ratzan"}
            ]
        }
    ]
}

export default lista


/*

,
{
    "id": ,
    "url": "",
    "invitados":[
    {"nombre": ""},
    {"nombre": ""},
    {"nombre": ""}
]
}


*/
